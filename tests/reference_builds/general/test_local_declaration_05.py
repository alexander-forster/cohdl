...
class test_local_declaration_05(cohdl.Entity):
    clk = Port.input(Bit)
    step = Port.input(Bit)
...
class Mock(MockBase):
    def __init__(self, dut: test_local_declaration_05, *, record=False):
...
@cocotb_util.test()
async def testbench_local_declaration_04(dut: test_local_declaration_05):
    mock = Mock(dut)
    mock.step <<= 0
    mock.check()
...
class Unittest(unittest.TestCase):
    def test_local_declaration_05(self):
        cocotb_util.run_cocotb_tests(
            test_local_declaration_05, __file__, self.__module__
        )
