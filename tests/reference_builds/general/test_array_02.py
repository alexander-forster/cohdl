...
class test_array_02(cohdl.Entity):
    ...
    rd_data = Port.output(BitVector[8])

    wr_addr = Port.input(BitVector[4])
    wr_data = Port.input(BitVector[8])
    ...
    def architecture(self):
        ...
        def proc():
            self.rd_data <<= mem[self.rd_addr.unsigned].lsb(8)
            mem[self.wr_addr.unsigned].unsigned <<= self.wr_data.unsigned
...
@cocotb_util.test()
async def testbench_functions(dut: test_array_02):
    seq_test = cocotb_util.SequentialTest(dut.clk)

    data_gen = cocotb_util.ConstrainedGenerator(8)
...
class Unittest(unittest.TestCase):
    def test_base(self):
        cocotb_util.run_cocotb_tests(test_array_02, __file__, self.__module__)
