...
class test_any_all_01(cohdl.Entity):
    ...
    def architecture(self):
        ...
        def logic():
            ...
            self.out_any_list <<= any(l)
            self.out_all_list <<= all(l)
...
@cocotb_util.test()
async def testbench_match_simple(dut: test_any_all_01):
...
class Unittest(unittest.TestCase):
    def test_base(self):
        cocotb_util.run_cocotb_tests(test_any_all_01, __file__, self.__module__)
