...
class SimpleContext:
    def __init__(self, enable, output, counter, level_monitor, output_cnt=1):
        self.enable = enable
        self.output = output
...
class test_async_context_manager_01(cohdl.Entity):
    clk = Port.input(Bit)
    ...
    def architecture(self):
        ...
        async def proc():
            ...
            async with SimpleContext(self.enable, self.ctx_a, level, self.level):
                with SimpleContext(self.enable, self.ctx_b, level, self.level) as b:
                    await std.wait_for(3)
                    assert b.output is self.ctx_b
                    async with SimpleContext(
                        self.enable, self.ctx_c, level, self.level, output_cnt=2
                    ) as (
                        c,
                        cc,
                    ):
                        ...
                        assert cc is c
                        async with SimpleContext(
                            self.enable, self.ctx_d, level, self.level
                        ) as d, SimpleContext(
                            self.enable, self.ctx_c, level, self.level, output_cnt=3
                        ) as (
                            c1,
                            c2,
                            c3,
...
@cocotb_util.test()
async def testbench_context_manager_01(dut: test_async_context_manager_01):
...
class Unittest(unittest.TestCase):
    def test_base(self):
        cocotb_util.run_cocotb_tests(
            test_async_context_manager_01, __file__, self.__module__
        )
