...
class test_array_nested_01(cohdl.Entity):
    clk = Port.input(Bit)
    ...
    def architecture(self):
        mem = Signal[Array[Array[BitVector[16], 4], 8]]()

        @std.sequential(std.Clock(self.clk))
        def proc():
            self.rd_data <<= mem[self.rd_addr_a.unsigned][self.rd_addr_b.unsigned]
            mem[self.wr_addr_a.unsigned][self.wr_addr_b.unsigned] <<= self.wr_data
...
@cocotb_util.test()
async def testbench_functions(dut: test_array_nested_01):
