...
class test_fifo_01(cohdl.Entity):
    clk = Port.input(Bit)
...
@cocotb_util.test()
async def testbench_fifo_01_single_push_pop(dut: test_fifo_01):
...
@cocotb_util.test()
async def testbench_fifo_01_full_empty(dut: test_fifo_01):
    tester = FifoTester(dut)
    await tester.reset()

    for prefill_cnt in range(10):
...
class Unittest(unittest.TestCase):
    def test_fifo_01(self):
        cocotb_util.run_cocotb_tests(test_fifo_01, __file__, self.__module__)
