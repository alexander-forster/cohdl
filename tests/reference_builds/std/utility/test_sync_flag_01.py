...
class test_sync_flag_01(cohdl.Entity):
    clk = Port.input(Bit)
...
class SyncTester:
    ...
    def __init__(self, dut: test_sync_flag_01):
        self.dut = dut
...
@cocotb_util.test()
async def testbench_sync_flag_01_mock(dut: test_sync_flag_01):
...
@cocotb_util.test()
async def testbench_sync_flag_01_mock_delay(dut: test_sync_flag_01):
    tester = SyncTester(dut)
    await tester.reset()

    for sender_delay_val in (0, 1, 2, 3, 4, 5):
        for receiver_delay_val in (0, 1, 2, 3, 4, 5):
...
class Unittest(unittest.TestCase):
    def test_sync_flag_01(self):
        cocotb_util.run_cocotb_tests(test_sync_flag_01, __file__, self.__module__)
