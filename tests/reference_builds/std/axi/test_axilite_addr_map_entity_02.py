...
class test_axilite_addr_map_entity_02(axi.addr_map_entity(addr_map="TestedAddrMap")):
...
@cocotb.test()
async def testbench_axilite(dut: test_axilite_addr_map_entity_02):
...
class Unittest(unittest.TestCase):
    def test_axilite_02(self):
        ...
        cohdl_testutil.run_cocotb_tests(
            test_axilite_addr_map_entity_02, __file__, self.__module__
        )

    def test_system_rdl(self):
        # basic test, checks that conversion to system rdl does not crash
        # output not verified
        to_system_rdl(test_axilite_addr_map_entity_02._gen_addr_map_())
