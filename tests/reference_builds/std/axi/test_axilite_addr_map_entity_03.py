...
class test_axilite_addr_map_entity_03(axi.addr_map_entity()):
    # check that addr_map_entities can be defined by overloading
    # _gen_addr_map_ function

    @classmethod
    def _gen_addr_map_(self):
        class TestedAddrMap(reg32.AddrMap):
...
@cocotb.test()
async def testbench_axilite(dut: test_axilite_addr_map_entity_03):
...
class Unittest(unittest.TestCase):
    def test_axilite_03(self):

        cohdl_testutil.run_cocotb_tests(
            test_axilite_addr_map_entity_03, __file__, self.__module__
        )

    def test_system_rdl(self):
        # basic test, checks that conversion to system rdl does not crash
        # output not verified
        to_system_rdl(test_axilite_addr_map_entity_03._gen_addr_map_())
