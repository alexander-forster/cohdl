...
class test_spi_ctx_01(Entity):
    ...
    def architecture(self):
        ...
        async def proc():
            ...
            with await spi_master.transaction_context(self.command[7:5]) as spi:
                await spi.send_data(self.command[4:0])
                self.led[7:4] <<= await spi.read_data(4)
                self.led[3:0] <<= await spi.read_data(4)
...
@cocotb_util.test()
async def testbench_spi_ctx_01(dut: test_spi_ctx_01):
...
class Unittest(unittest.TestCase):
    def test_spi_ctx_01(self):
        cocotb_util.run_cocotb_tests(test_spi_ctx_01, __file__, self.__module__)
