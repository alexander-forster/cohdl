    ...
    def architecture(self):
        ...
        async def proc():
            ...
            with await spi_master.transaction_context(self.command) as spi:
                self.led[7:1] <<= await spi.read_data(7)
                self.led[0:0] <<= await spi.read_data(1)


class SimpleSpiSlave(SpiSlaveBase):
    def __init__(self, signals):
        self._config = SpiConfig(cpha=True, cpol=True)
        self.content = 0
        self.data = 0
...
@cocotb_util.test()
async def testbench_spi_ctx_03(dut: test_spi_ctx_03):
...
class Unittest(unittest.TestCase):
    def test_spi_ctx_03(self):
        cocotb_util.run_cocotb_tests(test_spi_ctx_03, __file__, self.__module__)
