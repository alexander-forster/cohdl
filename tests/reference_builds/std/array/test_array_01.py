...
class test_array_01(cohdl.Entity):
    ...
    def architecture(self):
        ...
        memory = std.Array[BitVector[32], 32](Null)
...
class Mock(MockBase):
    def __init__(self, dut: test_array_01, *, record=False, no_assert=False):
...
@cocotb.test()
async def testbench_array_01(dut: test_array_01):
    mock = Mock(dut)
    mock.zero_inputs()
    await mock.tick()
...
class Unittest(unittest.TestCase):
    def test_array_01(self):
        cohdl_testutil.run_cocotb_tests(test_array_01, __file__, self.__module__)
