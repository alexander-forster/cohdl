...
import cohdl
from cohdl import BitVector, Port, Signal, Unsigned, vhdl
...
class VhdlTestWriter(TestWriter):
    ...
    def write(self, file=None):
        def vhdl_type(obj):
            ...
            if std.instance_check(obj, Unsigned):
                return f"unsigned({len(obj)-1} downto {0})"
            elif std.instance_check(obj, BitVector):
                return f"std_logic_vector({len(obj)-1} downto {0})"
            elif std.instance_check(obj, std.UFixed):
                return f"ufixed({obj.left()} downto {obj.right()})"
            raise
    ...
    def to_ufixed(self, obj, left, right):
        return f"to_ufixed({self._val(obj)}, {self._val(left)}, {self._val(right)})"
...
class CoHDL_TestWriter(TestWriter):
    ...
    def to_ufixed(self, obj, left, right):
        return std.UFixed[left:right](raw=obj.unsigned)
...
def test_fixed_math(t: TestWriter):
    ...
    for (l, r), sig in a_signals.items():
        t.assign(sig, t.to_ufixed(inp_a, l, r))

    for (l, r), sig in b_signals.items():
        t.assign(sig, t.to_ufixed(inp_b, l, r))
