...
class Mock(MockBase):
    def __init__(self, dut: test_if_02, *, record=False, no_assert=False):
...
@cocotb_util.test()
async def testbench_if_02(dut: test_if_02):
    mock = Mock(dut)
    mock.zero_inputs()
    mock.step <<= 0
    mock.check()
...
class Unittest(unittest.TestCase):
    def test_if_02(self):
        cocotb_util.run_cocotb_tests(test_if_02, __file__, self.__module__)
