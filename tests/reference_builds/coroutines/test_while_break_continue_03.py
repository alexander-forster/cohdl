...
class test_while_break_continue_03(cohdl.Entity):
    clk = Port.input(Bit)
    reset = Port.input(Bit)
    state = Port.output(Unsigned[3], default=Null)
    step = Port.input(Bit)

    do_continue = Port.input(Unsigned[2])
    ...
    def architecture(self):
        ...
        async def proc():
            ...
            while True:
                ...
                if self.do_continue == 0:
                    continue
                elif self.do_continue == 1:
                    continue
                elif self.do_continue == 2:
                    break
                else:
                    break
...
class Mock(MockBase):
    def __init__(
        self, dut: test_while_break_continue_03, *, record=False, no_assert=False
    ):
        ...
        self.do_continue = self.inpair(dut.do_continue, cv(2), "do_continue")
...
@cocotb_util.test()
async def testbench_while_break_continue_03(dut: test_while_break_continue_03):
    mock = Mock(dut)
    mock.zero_inputs()
    mock.check()
...
class Unittest(unittest.TestCase):
    def test_base(self):
        cocotb_util.run_cocotb_tests(
            test_while_break_continue_03, __file__, self.__module__
        )
