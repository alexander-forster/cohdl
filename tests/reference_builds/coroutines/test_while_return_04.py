...
class test_while_return_04(cohdl.Entity):
    clk = Port.input(Bit)
...
class Mock(MockBase):
    def __init__(self, dut: test_while_return_04, *, record=False, no_assert=False):
...
@cocotb_util.test()
async def testbench_while_return_04(dut: test_while_return_04):
    mock = Mock(dut)
    mock.zero_inputs()
    mock.check()
...
class Unittest(unittest.TestCase):
    def test_base(self):
        cocotb_util.run_cocotb_tests(test_while_return_04, __file__, self.__module__)
