...
class test_while_break_continue_01(cohdl.Entity):
    clk = Port.input(Bit)
...
class Mock(MockBase):
    def __init__(
        self, dut: test_while_break_continue_01, *, record=False, no_assert=False
    ):
...
class Unittest(unittest.TestCase):
    def test_base(self):
        cocotb_util.run_cocotb_tests(
            test_while_break_continue_01, __file__, self.__module__
        )
