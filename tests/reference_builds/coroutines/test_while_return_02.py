...
class test_while_return_02(cohdl.Entity):
    clk = Port.input(Bit)
        ...
        async def coro(cnt):
            while cnt:
                ...
                if self.do_return:
                    ...
                    # same timing as test_while_return_02 because
                    # the delay introduce by 'std.tick' and the skipped loop check
                    # due to 'continue' cancel out
                    await std.tick()
                    continue
...
class Mock(MockBase):
    def __init__(self, dut: test_while_return_02, *, record=False, no_assert=False):
...
@cocotb_util.test()
async def testbench_while_return_02(dut: test_while_return_02):
    mock = Mock(dut)
    mock.zero_inputs()
    mock.check()
...
class Unittest(unittest.TestCase):
    def test_base(self):
        cocotb_util.run_cocotb_tests(test_while_return_02, __file__, self.__module__)
