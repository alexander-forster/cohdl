...
import cohdl
from cohdl import Bit, Port, Unsigned, Null, true
...
class test_true_false_03(cohdl.Entity):
    clk = Port.input(Bit)
...
class Mock(MockBase):
    def __init__(self, dut: test_true_false_03, *, record=False, no_assert=False):
...
@cocotb_util.test()
async def testbench_true_false_03(dut: test_true_false_03):
    mock = Mock(dut)
    mock.zero_inputs()
    mock.check()
...
class Unittest(unittest.TestCase):
    def test_base(self):
        cocotb_util.run_cocotb_tests(test_true_false_03, __file__, self.__module__)
